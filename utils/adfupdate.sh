...
jq --argjson v0 "$akv_baseUrl" '.typeProperties.baseUrl = $v0' adf/linkedService/AzureKeyVault1-template.json >| adf/linkedService/AzureKeyVault1.json
...
--argjson v5 "$tenant" \
'.properties.typeProperties.appId = $v1 | .properties.typeProperties.mlWorkspaceName = $v2 | .properties.typeProperties.resourceGroupName = $v3 | .properties.typeProperties.subscriptionId = $v4 | .properties.typeProperties.tenant = $v5' adf/linkedService/AzureMLService1-template.json >| adf/linkedService/AzureMLService1.json
...
--argjson v2 "$env" \
--argjson v3 "$location" \
'.name = $v1 | .properties.globalParameters.environment.value = $v2 | .location = $v3 ' adf/factory/template.json >| adf/factory/${adfName//\"}-test.json

rm adf/factory/template.json
rm adf/factory/linkedService/AzureKeyVault1-template.json
rm adf/factory/linkedService/AzureMLService1-template.json
